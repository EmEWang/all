...
关键字noexcept
1 从C++11开始，很多代码当中都有关键字noexcept。比如下面就是std::initializer_list的默认构造函数。
  constexpr initializer_list() noexcept : _M_array(0), _M_len(0) { }
该关键字告诉编译器，函数中不会发生异常,这有利于编译器对程序做更多的优化。
如果运行时，noexecpt函数向外抛出了异常(如果函数内部捕捉了异常并完成处理，这种情况不算抛出异常)，程序会直接终止，
...
表示，如果类型T1和T2的移动分配过程中不发生异常，std::pair的移动分配函数(move assignment)不会发生异常。

4 什么时候该使用noexcept
使用noexcept表明函数或操作不会发生异常，会给编译器更大的优化空间。然而并不是加上noexcept就能提高效率。
以下情形鼓励使用noexcept：
移动构造函数(move constructor)
移动分配函数(move assignment)
析构函数(destructor)。
这里提一句，在新版本的编译器中，析构函数是默认加上noexcept的。下面代码可以检测编译器是否给析构函数加上noexcept。
  struct X {
      ~X() { };
  };
...
叶子函数(Leaf Function)是指在函数内部不分配栈空间，也不调用其它函数，也不存储非易失性寄存器，也不处理异常。
