...
// 参数1 为测试案例的前缀，可以任意取。     prefix
// 参数2 为测试案例的名称，需要和之前定义的参数化的类的名称相同。   test_case_name
// 参数3 为可以理解为参数生成器，上面的例子使用test::Values表示使用括号内的参数。
...
// ! 命名规则大概为：prefix/test_case_name.test_name/index
...
vector<struct params_add> vct_params_add = {{13,7},{1,19},{2,18},{22,-2}};
void func11()
{
    vct_params_add.push_back({1,19});
    vct_params_add.push_back({2,18});
    vct_params_add.push_back({4,16});
}
...
public:
    ...
    {
          // 每个test_suit 开头
          // std::cout << " ~~~~~~~~~~~~~~~SetUpTestCase size " << vct_params_add.size() << std::endl;
          vct_params_add.push_back({1,19});
          vct_params_add.push_back({2,18});
          vct_params_add.push_back({3,17});
    }
    ...
    {
        // 每个test_suit 结尾
        // std::cout << " ~~~~~~~~~~~~~~~TearDownTestCase size " << vct_params_add.size() << std::endl;
    }
protected:
    void SetUp()
    {
        // 每个test 开头
        // std::cout << " ~~~~~~~~~~~~~~~SetUp size " << vct_params_add.size() << std::endl;
        vct_params_add.push_back({1,19});
        vct_params_add.push_back({2,18});
        vct_params_add.push_back({3,17});
    }
    ...
    {
        // 每个test 结尾
        // std::cout << " ~~~~~~~~~~~~~~~TearDown size " << vct_params_add.size() << std::endl;
    }

};

TEST_P(IsAddTest, testx)
{
    ...
    // }
    // std::cout << " ~~~~~~~~~~~~~~~size " << vct_params_add.size() << std::endl;
    // func11();
...
// ! 利用逗号表达式，动态改变变量的值 原理是先取值在运行测试案例
// INSTANTIATE_TEST_CASE_P(IsAddTestParams, IsAddTest, testing::ValuesIn((func11(),vct_params_add)));
...
{
public:
    CallArgs(bool hasAudio,bool hasVideo):_hasAudio(hasAudio),_hasVideo(hasVideo){}
...
INSTANTIATE_TEST_CASE_P(VOIP, Call, ::testing::Values(CallArgs(true,true),CallArgs(true,false)));
