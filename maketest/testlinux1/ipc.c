...
// 消息队列   ipcs -q 显示     ipcrm -q id 按id删除 ipcrm -a msg 删除所有
#include <sys/msg.h>
void testmsg_send();
void testmsg_recv();

// 信号量     ipcs -s 显示     ipcrm -s id 按id删除 ipcrm -a sem 删除所有
#include <sys/sem.h>
void testsem();

// 共享内存   ipcs -m 显示     ipcrm -m id 按id删除 ipcrm -a shm 删除所有
#include <sys/shm.h>
void testshmsrver();
void testshmcilent();
...
// write 端关闭   read 返回 0
// read  端关闭  write 返回 -1 产生信号 SIGPIPE  errno置为EPIPE
// PIPE_BUF 为内核管道缓冲区大小  pathconf  fpathconf 可查询
void testpipe()
{
    int fd[2];
...
void testpopen2()
{
    ...
    {
        fputs("prompt:", stdout);
        fflush(stdout);   // 连接之后变成全缓冲 需要fflush 立即呈现
...
void testsem()
{
    ...
    printf("uid = %d,mode = %o,sem_otime = %ld,sem_nsems = %ld\n",
        semds.sem_perm.uid,semds.sem_perm.mode,semds.sem_otime,semds.sem_nsems);
    // semval 当前的信号量的值
    printf("semval0 = %2d,semncnt0 = %2d\n",semctl(sem_id, 0, GETVAL), semctl(sem_id, 0, GETNCNT));
    printf("semval1 = %2d,semncnt1 = %2d\n",semctl(sem_id, 1, GETVAL), semctl(sem_id, 1, GETNCNT));
...
//  消息队列是随内核的持续性，即一个进程向消息队列写入消息后，然后终止，另外一个进程可以在以后某个时刻打开该队列读取消息。
//  只要内核没有重新自举，消息队列没有被删除。
