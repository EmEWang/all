...
for((i=0;i<=num;i++))
{
  ${jeprof} --svg ${app} ${prefix}.${moment[$i]}.i${moment[$i]}.heap > ${out}/${out_name}.moment.[$startindex-$endindex][${moment[$i]}].svg
...
{
  j=$((i-1))
  ${jeprof} --svg ${app} --base=${prefix}.${moment[$j]}.i${moment[$j]}.heap ${prefix}.${moment[$i]}.i${moment[$i]}.heap > ${out}/${out_name}.diff.[$startindex-$endindex][${moment[$j]}-${moment[$i]}].svg
