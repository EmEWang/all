...
class SymbolResolver:
    ...
    def resolve_symbols_batch(self, pcs: [int], *, simplify_symbol: bool = False, annotate_libname: bool = False) -> tp.Dict[int, str]:
...
class FlamegraphData:
    ...
    def write_svg_ouput(self, filepath: Path, flamepath,flamegraph_args: tp.List[str] = []):
        type(filepath)
        subprocess.run(
            [flamepath+'/FlameGraph/flamegraph.pl', *self._default_flamegraph_args, *flamegraph_args],
            input=self._data,
