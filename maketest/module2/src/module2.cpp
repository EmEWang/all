...
// <ESC>[{ROW};{COLUMN}f  Force Cursor Position    ESC is the byte 0x1B
void gotoxy(int x,int y){
    printf("%c[%d;%dH",0x1B,y,x); // 或  printf("%c[%d;%df",0x1B,y,x);
}
...
namespace wjg
{
    ...
    int tree_simple_avl::print(int y, bool clear)
    {
        if (!m_root)
        {
            return 1;
    ...
    void tree_simple_avl::print2()
    {
        ...
        char strmap[12][1024] = {' '};
        int depth = 0;
        // print2(m_root, strmap, depth, benchmark);
        // print2(m_root, strmap);
        // print2(m_root, strmap, len, skip, x, y, x);
        int height = this->height(m_root);
        for (int i= 0;i< lenmax;i++)
            printf("-");
        printf("\n");
    }
    void tree_simple_avl::print2(node_simple* node, char** strmap, int len, int skip, float x, int y, float step)
    {
        if (!node)
            return;
        char buff[24] = {0};
        sprintf(buff, "%d", node->key);
        // memcpy(strmap[y], buff, strlen(buff));
        // strmap[][y] = str;
