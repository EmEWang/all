#ifndef __MODULE3_STLTEMPLATE0_H__
#define __MODULE3_STLTEMPLATE0_H__
...
Meta 这个词根有着 “更高一层抽象” 的意味。例如我们在数据结构设计中提到的 Metadata 常指 “描述数据的数据”，在 Python 中使用的 Metaclass 是 “描述 class 的 class”，Metaprogramming 也有类似的意思。

元编程（Metaprogramming）就是编写一类特殊程序的编程，这类程序将其他程序或者其自身视为输入的数据来进行处理，我们称这类特殊的程序为元程序（Metaprogramming）。

Metaprogramming is the writing of computer programs that write or manipulate other programs (or themselves) as their data.[1]
对于元程序，有个更诗意的描述：
...
Compilers are computer programs that translate a program written in one language into a program written in another language.[3]
我们举一个更具体的例子，YACC 是一个语法分析器生成器（Parser Generator）， 它的输入是一份对语法（Grammar）的描述，输出是一个语法分析器（Parser）。
...
这里有趣的一点是，Parser 本身也是一个 Metaprogram，YACC 是生成 Metaprogram 的 Metaprogram， 这与 YACC 的全称不谋而合：Yet Another Compiler-Compiler。在这个例子中，YACC 处理的代码 grammar.y 是用一种专门的语言 Context Free Grammar (CFG) 描述的，YACC 将之转译为 C 后再由 C 编译器处理。在这里，我们称 CFG 为 Domain Language，称 C 为 Host Language。我们根据是否使用了 Domain Language 来实现元编程，可以将元编程技术分为两类：

（1）Metaprogramming with Domain Language
在这类元编程中，元程序定义一种专门的语言（Domain Language），逻辑代码是用 Domain Language 书写的，然后被元程序翻译为目标语言（Host Langugage）再去编译执行。逻辑代码（无论是用 Domain Language 还是 Host Language 表示）与元程序的代码是分离的。



（2）Metaprogramming in Host Language
在这类元编程中，逻辑代码和元程序自身的代码是写在一起的，用同一种语言，元程序代码通过某种机制（通常是编译）转变为（或者说生成）逻辑代码，并与其他逻辑代码合并到一起。这就产生一种效果，这个元程序看起来像是在自己改写自己。

C++ 模板元编程（C++ Template Metaprogramming, TMP）就是第二种类型，TMP 的代码与普通的 C++ 代码写在一起，但 TMP 的逻辑在编译期执行，而普通 C++ 代码的逻辑在运行期执行。

1.2 What is C++ Template Metaprogramming?
C++ Template Metaprogramming（TMP）顾名思义是利用 C++ 模板进行的元编程。借助编译器处理模板的能力，TMP 可以在编译期间生成代码，这些代码最终和普通的 C++ 代码一起编译执行。我们举一个例子来直观地展示什么是 TMP：
...
在上面的代码中，我们先是定义了一个名为 binary 的模板，然后定义了一个 binary 的特化 (Specialization)。关于模板的特化我们会在后面详细介绍，这里可以将它理解为当 N==0 时对应的特殊版本。利用这段代码，我们可以在编译期计算出一个二进制数对应的十进制值：
...
binary<x>::value 在编译期就会被求值，得到一个常量。对模板不熟悉的读者可能会对这一点产生怀疑，我们可以用下面的代码来证明 binary<x>::value 是一个常量，如果不是，那么这句话在编译时就会报错，因为 C++ 要求在编译时就确定静态数组的长度。（在 C++ 中，如果一个东西的名字里包含 static，那么它大概率是编译期就决定的东西，如static_assert，static_cast；同样的，dynamic 通常意味着运行时才确定的事物，如 dynamic_cast, dynamic_link）

static char array[binary<101>::value];     // Equivalent to: static char array[5]
我们在这里暂时不去探究 binary 作用的原理，稍后在学习了模板的基础知识后，我们再回来看 binary 是怎么起作用的。重要的是我们通过这个例子看到了 TMP 在编译期计算的能力。这种能力不是空中楼阁，C++ 模板已经被证明了是图灵完备的，这意味着可由计算机程序表达的任何计算都可以通过 TPM 来完成。

1.3 History of TMP
有意思的是，C++ 的这种在编译时计算的能力，并非是有意设计发明的，而是“偶然”发现的。1994 年，C++ 标准委员会在圣地亚哥举行的一次会议期间，Erwin Unruh 展示了一个可以产生质数的程序。这个程序的特别之处在于质数的产生发生于编译期而非运行期，它在编译器产生的一系列错误提示中打印出了 2 到某个指定值之间的所有质数。这个程序被认为是 TMP 的起源，它第一次向世人展示了 TMP 的能力。从根源上来说，这种在编译期计算的能力源自于 C++ 标准对模板的描述。对于这段历史，更详细的描述可以参看 Erwin Unruh 的个人回忆。在 TMP 之前，STL 的泛型容器和算法被认为是模板编程的巅峰之作，TMP 使得人们重新认识了模板的能力。

与 C++ 的很多技术一样，TMP 的发展是由社区推动的。自 Erwin Unruh 之后，许多人都开始研究 TMP，并在他们的项目中使用 TMP 相关的技术。其中最典型是一系列 Metaprogramming Library 的出现，这些 Library 提供编译期的容器和算法，就像是 STL 一样。不同的是这些容器存储的是常量和类型，这些算法都是运行在编译期间。比较知名的 Metaprogramming Library 有：
...
对许多人来说，在编译期计算听起来不仅匪夷所思，而且毫无必要：要算质数，我在运行时计算不好吗？对我来说，学习和使用 TMP 的理由主要有下面四个：

一：TMP 可以做编译期的计算
这使得我们能将一些运行时的工作迁移到编译期，从而提升程序的运行时效率。当然这也是有代价的，通常，将工作迁移到编译期会导致编译的时间变长，以及目标文件(object file) 的大小增大。但因为我们的程序是一次编译多次运行的，所以有时候以编译期的代价换取运行时的效率是非常划算的。这有点类似于渲染中“烘焙”的技术，虽然烘焙场景需要很长时间，烘焙出的资源文件也会使游戏包体变大，但它在运行时的性能优化是我们更想要的。
二：TMP 可以对类型(type)做计算
如果说性能优化还可能通过别的方式实现，那么 TMP 对类型做计算的能力则是无可替代的了。众所周知，C++ 缺乏有效的自省机制，在运行时根据变量的类型去编写逻辑是不可能的。但在编译期，类型可以作为模板的实参(Arguments) 参与逻辑运算，这使得我们可以写出类似下面这样的代码：

// pseudo code
if (typeof(var) == int) { ... }
else { ... }
三：TMP 可以使代码变得简洁优美
TMP 自身的代码是晦涩难懂的，甚至可以说是 “丑陋” 的，但有时它却可以使使用它的外部代码变得简洁优雅。例如下面是使用 pybind11 导出一个 C++ 类到 Python 的代码，如果没有 TMP，pybind11 的接口不可能如此简洁明了：
...
四：TMP 的广泛应用
正如我前面所说的，TMP 已经是一个广泛使用且不可或缺的技术了，即使你没有直接写过 TMP 的代码，也一定曾间接地从 TMP 中受益。很多程序库，特别是泛型程序库，都几乎不可避免地使用了 TMP，例如 Standard Library，asio, pybind11, boost::python 等。如果你不了解 TMP，你可能都看不懂这些库中的代码，那更别提深度使用它们了。
...
is_same 是一个类模板，它有两个模板形参，T 和 U，它的主模板继承了 false_type，另外有一个特化继承了 true_type，这个特化模板匹配 T 和 U 相同的情况。这个过程和我们之前描述的一样，当 T 和 U 相同时，编译器将 T 和 U 代入模板特化的实参列表里，然后尝试推导特化模板的形参，因为两个参数相同，所以推导得出一致的结果，匹配特化成功，is_same<T, U>::value == true。当 T 和 U 不同时，推导失败，fallback 到匹配主模板，这时 is_same<T, U>::value == false。
...
is_one_of 的主模板形参分为三个部分：类型 T，类型 U，和变长形参列表（Parameter Pack）Rest，另有一个特化模板，只接受两个参数 T 和 U，这个特化的逻辑等效于 is_same。主模板的递归逻辑是一个析取表达式，判定 T 和 U 是否相等，或者判定 T 是不是包含在剩余的参数中。这里的 Rest... 是对变长形参列表的展开，当我们要引用一个变长形参列表内的内容时，就需要这样写。这个递归会一直持续遍历所有的参数，直到只剩下 T 和 最后一个参数，这时匹配模板特化，递归终止。
...
rank 包含一个主模板和两个偏特化。根据模板特化的匹配规则我们知道，当模板实参是数组类型时，会匹配 #2 或 #3 这两个特化，当实参是非数组类型时，匹配主模板。其中，#2 匹配不定长数组；#3 匹配定长数组。整个递归的过程就是对维度做递归，每次递归 value + 1，就可以得到总维度。这里我们可以思考一个问题：对于 int[5][6]，毫无疑问会匹配到特化 #3，那么这时 #3 的两个参数 T 和 N 被推导为什么呢？是 int[5] 和 6，还是 int[6] 和 5？答案我不公布了，你可以尝试写代码来测试看看。
...
4. Template in Depth
读到这里，恭喜你，你对 TMP 已经有不错的认知了！但我们还没充分展示 TMP 的威力，比如前面提到的 Function Template 都还没派上用场。本章中我们将了解一些进阶的模板知识，并在下一章将它们应用到例子中去。

4.1 Template Arguments Deduction of Function Template
函数模板的实参推导发生在名字查找（Name Lookup）之后，重载决议（Overload Resolution）之前。如果函数模板实参推导失败，那么编译器不会直接报错，而是将这个模板从函数的重载集中无声地删除。[4][5]
...
例如，在上面的例子中，foo 有两个函数模板和一个普通函数，对 #2 的实参推导失败了，但并不会发生错误，编译器会匹配模板 #1。我们简单地阐述一下这个过程中发生了什么：
...
类模板的实参推导与函数模板不同，当同时定义了主模板和模板特化时，实参推导只考虑主模板，模板特化不参与实参推导。[6]。如果主模板实参推导出错，那么编译器直接报错。
...
例如在上面的例子中，编译器通过主模板的构造函数推导出 T=int, U=float，这个模板实参的最佳匹配结果是特化#2，所以最终调用的构造函数是 #2 实例化后的 S<int, float>::S(int, int)，第二个实参 1.0f 被隐式转型为了 int。而假设你用特化 #2 去做模板实参推导的话，这个推导是失败的，这也反证了编译器并不是用模板特化推导实参的。我们也简单阐述一下这里发生的事：
...
在模板的实例化（Instantiation）中，当所有的模板实参都确定了之后（这些实参可能是显式指定的、推导的、或者从默认实参中获取的），编译器就需要在模板的主模板和所有显式特化中选择一个来做替换（Substitution）。规则是：

对每一个模板特化，先判断它能不能匹配该实例化（判断过程我们在3.1.1 is_reference中讲了）。
如果只有一个模板特化能匹配模板实参，那么就选择这个特化；
如果有多个模板特化都能匹配模板实参，那么通过这些特化的偏序关系来决定哪个模板特化的特化程度更高，其中特化程度最高的那个将会被选中。如果特化程度最高的模板特化不止一个，也就是说，存在多个模板特化，它们的特化程度无法比较高低，那么编译器会报错，实例化失败。
...
然后，模板特化 A 和 B 的优先级规则，就被转换为了函数模板重载的优先级规则了。这个规则就是 Partial Ordering Rule，我们在下一节介绍。在这里我们看到，通过一个巧妙的转换，特化决议和重载决议对优先级的排序就被归一到了同一个算法里，它们是一致的。

4.4 Partial Ordering Rule
对于两个函数模板 fa 和 fb，怎么判定它们两个谁的特化程度更高呢？这个过程也是一个代入+推导的过程。为了方便，我们定义一个过程名字就叫“代入推导”（这个名字我自己起的，方便我阐述），用 fa 代入推导 fb 是指：

记 fb 的模板形参为 T。假设 fa 的模板实参是 U，那么 fa 的函数实参类型（记为A）就可以用 U 来表示出来。我们用 A 去代入 fb 的函数形参列表（记为P），并且尝试由此推导 fb 的模板形参 T，这里推导的意思就是尝试用 U 来表示出 T。
...
规则有点复杂，我们还是通过例子来理解。我们尝试对比 #1 和 #2 的偏序关系。首先，尝试用 #2 来代入推导 #1，我们记 #1 的模板形参为 T，并假设给 #2 传入一个实参 U，那么 #1 和 #2 就被表示为这样两个函数模板：
...
template <typename T>   void foo(T*); // #2
然后将上面的函数实参 U 代入下面，也就是将 #2 写为 template <typename T> void foo(T*=U)，然后推导T，失败。所以我们得出结论：#2 的特化程度比 #1 高。

这个规则之所以称为“偏序”规则，是因为函数模板的重载集（或类模板的偏特化集）是一个偏序集合，集合元素之间的关系是偏序关系。也就是说，并不是任意两个函数模板都是可比较的，有时你无法比较两个模板谁的特化程度更高。如果从 fa 代入推导 fb 成功，且从 fb 代入推导 fa 成功；或者从 fa 代入推导 fb 失败，且从 fb 代入推导 fa 失败，那么 fa 和 fb 就被认为是无法比较的，或者说特化程度相同的。例如：
...
答案是，函数模板的每一个重载都是主模板（Primary Template），在重载决议的时候，只考虑非模板函数和函数模板的主模板，模板的特化不在重载集的范畴之内。并且，对于一个函数调用，是先进行重载决议，确定使用哪个主模板，再考虑是否使用该模板的特化。
...
答案是 #3。就像我们说的，编译器会在 #1 和 #3 中先进行重载决议，#2 只是 #1 的一个特化，不予考虑。重载决议选择了 #3，而且 #3 也没有特化，那么就会实例化 #3 这个主模板。虽然我们能看出来 #2 才是这次调用的一个完美匹配，但很可惜，规则如此。
...
template <>           void foo(int*);  // #2
foo(new int(1));  // which one will be called?
这个例子只是把 #2 和 #3 调换了一下位置，答案就变了，这次编译器会选择 #2。因为调换了位置之后，#2 变成了 #3 的一个特化了。这里我们看到，编译器在决定一个模板特化属于哪一个主模板的时候，只会从它已经“看见”的主模板里选择。在第一个例子中，编译器在为 #2 寻找主模板的时候，还没看见 #3，所以认为它是 #1 的特化。
...
最后，值得说明的是，编译器在重载决议的时候为什么不考虑特化呢？因为模板的特化并不引入一个名字（Name）。所以在名字查找的时候，模板特化就直接是被忽略的，或者说是和主模板视为一体的，它不是一个独立的名字。还记得我们在 4.2 Template Arguments Deduction of Class Template 中讲的吗？类模板的实参推导不考虑其特化，归根结底也是同一个原因：模板的特化并不引入一个名字。 所以我们一定要注意，编译器是按步骤做事的，第一步，名字查找；第二步，实参推导；第三步，重载决议；第四步，选择特化；第五步，做替换生成实例。

4.6 SFINAE
SFINAE 的全程是：Substitution Failure Is Not An Error，替换失败不是一个错误。这可能是模板编程中最出名的一个规则了，你可能已经听过。替换失败不是一个错误，什么意思呢？我们将这句话拆分为两半来理解：
...
foo(1);
在这个实例化中，有三处地方发生了替换，一个是函数的返回值，一个是函数的形参列表，一个是函数内部的局部变量声明。在函数的返回值处，替换后构成了 “ill-formed” 的程序，因为 int::value 是一个非法的名字，但这里是在立即上下文内，所以是一个替换失败。在局部变量 i 的声明处，替换后也构成了 “ill-formed” 的程序，但这里不在立即上下文内，所以是一个硬错误（Hard Error）。
...
在 S<int, int> 的实例化中，#2 发生替换失败，编译器选择主模板 #1 来实例化。在 S<true_type, bool> 的实例化中，没有替换失败发生，编译器选择 #2。在 S<true_type, int> 的实例化中，没有替换失败发生，但根据匹配规则，#2 不匹配，所以选择 #1.
...
5.1.1 enable_if
假设我们想要这样的函数模板重载，对于整型的参数，匹配第一个模板，这个模板实现一些针对整型参数的逻辑；对于浮点数型的参数，匹配第二个模板，这个模板实现针对浮点型参数的逻辑。于是我们定义了下面两个函数模板重载：
...
但是这时我们发现，编译器报告了一个重定义错误，上面两个模板实际上是一模一样的，因为它们的签名完全相同。我们无法写一个针对参数类型的函数模板重载。唯一的办法是对每一个类型，int, char, long, float, double... 都写一个普通函数重载：
...
foo(1);     // match #1
foo(1.0f);  // match #2
我们先来看 enable_if 是怎么定义的。首先它是一个类模板，主模板有两个形参，第一个形参接受一个 bool 值，第二个形参接受一个类型 T 且有一个默认值 void，主模板就返回 T 本身。另外有一个偏特化，偏特化接受一个类型形参 T，它匹配 bool 值为 false 的情况，但注意，它内部没有定义 “type”！这就造成一种效果，当我们 “调用” enable_if<bool, T>::type 时，如果 bool 值为 true，那么 enable_if 就返回 T；而如果 bool 值为 false，enable_if 返回不了任何东西！“type” 不存在，对它的调用构成了一个非良构的（ill-formed）表达式，只要这个表达式位于立即上下文（Immediate Context）中，那么就会触发 SFINAE 机制！

所以，对于 foo(1)，在函数模板 #2 中，T 被推导为 int，is_floating_point_v<T> 为 false，所以 enable_if_t<is_floating_point_v<T>> 产生了非良构的实例化表达式。我们看下这个表达式位于哪里，它位于函数模板 foo 的返回值处，这里是一个立即上下文，所以这是一个替换失败（Substitution Failure），函数模板 #2 从重载集中剔除，避免了重定义错误。

foo(1.0f) 也是同理。所以，通过 enable_if（更准确地说是通过 SFINAE），我们拥有了基于逻辑来控制函数重载集的能力，而原本的我们仅仅只能基于 C++ 语法规则来控制重载，这就是 TMP 的威力。
...
S<int>::foo(1);
在这里，foo 是 S 的静态成员函数模板，我们希望，当 S 的实参是 int 时，匹配第一个 foo；当 S 的实参不是 int 时，匹配第二个 foo。也就是说，我们希望 S<int>::foo(1) 调用的是 #1。但是，我们去编译这段代码却发现编译器报告了错误：
...
我们分析下错误信息，编译器说在实例化 S<int> 的时候，enable_if<false> 里面没有名叫 “type” 的类型。这不是我们预期的替换失败吗？怎么没有 SFINAE，而是报告了一个错误？历史经验告诉我们，当编译器和我们的预期不一致时，一般都是我们错了。那么是哪里搞错了呢？仔细分析我们发现，原来是因为这个 enable_if 不在类模板 S 的立即上下文里！奇怪，enable_if 不是在 foo 的返回值里的吗？在上个例子里返回值不是立即上下文吗？注意，foo 的返回值属于立即上下文，是对 foo 来说的。也就是说，在实例化成员函数模板 foo 的时候，foo 的返回值区域位于立即上下文中。而我们现在正在实例化 S，只有在 S 的自己的立即上下文内才能使用 SFINAE。

所以我们就没办法通过类模板的实参控制成员函数重载了吗？不是的，有一个巧妙的办法能避免上面的错误。理论上，我们只需要将 enable_if 的实例化推迟到 foo 的立即上下文中就行了，让 enable_if 的实例化发生在 foo 实例化的时候，而不是 S 实例化的时候。我们看下面这段代码：
...
S<int>::foo(1);  // works
我们实现一个无意义的 Metafunction，always_true，它接受任意的参数，然后返回 true。然后，我们将 always_true_v<U> 放到 enable_if 的实参表达式里，U 是成员函数模板 foo 的形参，因为 always_true 永远是 true，所以这个合取表达式（&&）和原来的逻辑是一样的。神奇，这样问题就解决了！

我们来解释下原理，在实例化 S<int> 时，编译器用 int 替换 T，第二个 foo 中的 !is_same_v<T, int> 就变为了 false。但是对于这个合取表达式的剩余部分 always_true_v<U>，其值依赖于模板 foo 的形参 U，而这个 U 直到 foo 实例化的时候编译器才能确定它的值，所以编译器无法继续对 enable_if_t 求值了，所以也就没有任何非良构的表达式产生。直到 foo(1) 开始进行实例化的时候，U 被推导为 int，always_true_v<U> 返回 true。这时 enable_if_t<true && false> 就构成了非良构的，但是因为我们现在位于 foo 的立即上下文中，SFINAE！

另外，就像我们在 3.3.2 is_one_of 中提到的那样，模板的实例化是没有短路求值规则的，所以这里即使把 always_true_v<U> 放在 !is_same_v<T, int> 的后面，也是可以把 enable_if 的实例化推迟到 foo 实例化之时的。

5.1.2 void_t
如果 enable_if 还没有让你大脑宕机，那试一下 void_t 吧！void_t 的定义如下：

template <typename...> using void_t = void;
看起来是一个很简单的东西，这就是一个别名模板，接受任意的类型形参，不论你传给它什么，它都返回给你一个 void。这样一个东西能有啥用呢？说出来你可能不信，用它我们可以在 C++ 中实现一个类似 Python 里 hasattr 一样的东西。

假设我们现在想实现这样一个功能，要检查一个 Metafunction 是否遵守了 Metafunction Convention。也就是说，给一个任意的类型，我们检查它内部是否定义了一个名为 “type” 的名字：
...
我们定义了一个 Metafunction 叫做 has_type_member，它的主模板接受两个参数，第一个参数是一个类型，第二个参数也是一个类型，但默认值置为了 void。另有一个偏特化，偏特化保留了第一个参数 T，第二个参数期望匹配的是一个 void 类型，但我们没有直接写 void，而是写了 void_t<typename T::type>。

第一个关键点来了，我们先理解一下偏序规则是怎么在这里发挥作用的，也就是说，什么情况下，会匹配 has_type_member 的偏特化。对于第二个模板参数，当我们传入 void 时，由于主模板的能匹配任意的类型，而偏特化只匹配 void 类型，所以显然这时会匹配偏特化；当我们给第二个参数传入非 void 时，由于偏特化不匹配非 void，所以这时会匹配主模板。而重点是，当我们不传第二个参数时，那么编译器会从该形参的默认实参来确定实参，而默认实参就是 void。也就是说，当我们不显示指定第二个实参时，模板的偏序规则会优先匹配 has_type_member 的偏特化。

第二个关键点是，在偏特化的第二个特化形参里，并不是直接指定了 void，而是指定的一个 void_t 实例化表达式，这个表达式是有可能是非良构的！具体地说，当第一个形参 T 中包含名为 “type” 的成员、且 “type” 是一个类型时，表达式 typename T::type 良构；而当第一个形参 T 中不包含 “type” 成员或 “type” 不是一个类型时，表达式 typename T::type 非良构。这个非良构发生在 has_type_member 的形参列表里，属于立即上下文，因此 SFINAE 发挥作用。也就是说，当 T 中不包含 “type” 类型时，偏特化被剔除，这次实例化只能匹配has_type_member 的主模板。

所以，我们使用 has_type_member 时，只指定一个参数，不指定第二个参数。效果就是，当第一个参数里包含 “type” 成员时，匹配特化，结果为 true；当第一个参数里不包含 “type” 成员时，匹配主模板，结果为 false。
...
在 C++ 中，有四个运算符，它们的操作数是不会被求值（Evaluate）的，因为这四个运算符只是对操作数的编译期属性进行访问。[7] 这四个运算符分别是：typeid, sizeof, noexcept, 以及decltype。

std::size_t n = sizeof(std::cout << 42);  // Nothing is outputed
decltype(foo()) r = foo();                // Function foo is called only once.
例如，第一句里的 std::cout << 42 并不会被执行，第二句中的 foo 也只调用了一次，也就是说，这些运算符的操作数是不会在运行时生效的，它们甚至都不存在了，在编译期间就已经处理掉了。这些运算符的表达式称为不求值表达式（Unevaluated Expression），它们的参数所处的区域称为不求值上下文（Unevaluated Context）。
...
第一个模板匹配整型的实参 T，接受一个 T 类型的变量作为函数参数，返回值类型为 int；第二个模板匹配浮点数类型的实参 T，接受一个 T 类型的变量作为函数参数，返回值类型为 float。然后，我们定义一个类模板 S，它内部有一个成员 value_，我们希望 value_ 的类型是 S 的形参 T 对应的 foo 重载的返回值类型：

template <typename T> struct S { decltype(foo<T>(??)) value_; };  // What should be put in ??
为了得到 foo 的返回值类型，我们只需要写一个 foo 的调用表达式，然后将这个表达式传入 decltype 运算符，就能得到 foo 的返回值类型。并且根据我们知道 decltype 是一个不求值运算符，foo 的调用表达式并不会被求值。但是问题在于，foo 函数接受一个类型为 T 的变量作为参数，我们去哪里创建一个 T 的变量出来呢？况且，在编译期也没变量啊。
...
declval 是一个函数模板，首先我们注意到，它只有声明，没有定义。因为在不求值上下文中，对这个模板的实例化不会被求值，我们不是真的要创建这个变量，只是要让编译器假设我们有一个这样的变量。所以 declval 是不能被用在需要求值的地方的，它只能应用在不求值上下文中。另外，它的返回值类型是 T 的右值引用，add_rvalue_reference 也是一个 Metafunction，它把接受 T，返回 T 的右值引用，我们下节再介绍 add_rvalue_reference 的实现，现在只需要知道它的功能。有了 declval，我们就可以伪造一个变量传给 foo 了：
...
结合不求值表达式和 SFINAE，我们能实现更复杂的功能。还记得 remove_reference 吗？它移除一个类型的引用，现在我们反其道而行之，想实现两个 Metafunction，它们给类型加上引用。特别地，add_lvalue_reference 给类型加上左值引用，add_rvalue_reference 给类型加上右值引用：
...
这两个 Metafunction 很简单，相信大家都能看懂，这段代码看起来似乎以为没什么问题。等等，真的没问题吗？哦！如果我传入一个 void 会怎样？void 是没有相应的引用类型的，如果 T 是 void，那么将产生一个编译错误！那怎么办呢，能不能想个办法，当 T 是 void 时就返回 void 本身，而不是尝试返回 void 的引用呢？emmmm，对 void 进行特殊判断是一个办法，但是万一有别的类型也不支持添加引用呢？我们有一个更好的办法解决这个问题：
...
我们来解释一下这里的原理。对 add_lvalue_reference，我们实现两个辅助函数模板：type_identity<T&> detail::try_add_lvalue_reference(int) 和 type_identity<T> detail::try_add_rvalue_reference(...)。先看它们的返回值类型，他们的返回值类型一个是 type_identity<T&>， 一个是 type_identity<T>。而这个返回值类型决定了 add_lvalue_reference 的结果，因为是直接继承过来的。而具体继承哪个呢？取决于重载决议的结果。当 T=void 时，type_identity<void&> 是一个非良构的表达式，根据 SFINAE，第一个辅助模板被剔除，所以 add_lvalue_reference 实际继承的是第二个辅助函数模板的返回值，也就是 type_identity<void>，返回 void。而当 T=char 时，type_identity<char&> 是一个良构的表达式，这时两个函数模板都合法，那么根据偏序规则，由于第一个函数模板的函数形参类型特化程度更高，更匹配 detail::try_add_lvalue_reference<char>(0) 这个调用，所以第一个重载被选中，add_lvalue_reference 返回 char&。

可以看到，这种实现方式的思路是，不论 T 能否添加引用，我先假设能，先给它添上引用，反正也不求值，如果没错就最好，错了就 SFINAE。所以这种实现方式是更通用的，如果除了 void 外，有其他类型也不能添加引用的，这个实现也能覆盖到。总之思路就是管他行不行，我先试试，不行再说。
...
上面 decltype 括号里的就是一个赋值表达式，只不过我们是借助 declval 将它写出来的。对于一个赋值表达式，等号左边是一个 T& 类型的变量，等号右边是一个 T const& 类型的变量（或者说常量？），并且赋值表达式的返回值类型等于等号左边变量的类型，也就是 T&。如果赋值这句话良构，那么 copy_assign_t 就等于 T&；如果非良构，那么我们只要保证它在立即上下文里就行了，就可以 SFINAE。所以：
...
5.3.1 std::tuple
std::tuple 类似于 Python 的 tuple，可以存储多个不同类型的值。在 Python 这种弱类型的语言中，tuple 并不难理解；但 C++ 是强类型的语言，不同类型的值是如何存储到一个列表的呢？
...
tuple 的主模板什么都不做，只是定义了一个构造函数，这个构造函数也没有任何逻辑，定义它是为了做类模板实参推导。tuple 的模板特化实现了一个递归继承，tuple<T, Args...> 继承了 tuple<Args...>，这个递归继承会一直继承到 tuple<>，这时匹配主模板，递归终止。特化模板定义了一个类型为 T 的成员 value_，也就是说，在递归继承的每一层都存储了一个 value，每一层的 value 的类型都可以是不同的，这就是 tuple 可以存储不同类型变量的关键。如果你熟悉 C++ 的对象模型，那么你应该不难理解这些 value 是怎么存储的，它们在一块连续的内存空间内紧密排布，每一层根据其类型占据相应大小的空间。
...
和我们在 example 3.4.2: extent 中做的事情类似，tuple_element 也是对下标 N 做递归，直到 N=0，这时 T 就是第 N 个元素的类型。但有一点点特别的是，这里除了 tuple_element<>::type 以外，我们还定义了一个 tuple_element<>::__tuple_type，它代表的是从 N 之后的参数组成的 tuple 类型。例如，tuple_element<1, tuple<int, float, char>>::__tuple_type 就等于 tuple<float, char>。
...
因为 tuple 是一层层继承的，所以这里对 t 相当于是一个向上转型，转型后直接返回这一层的 value 就行了。另外 get 返回的是 value 的左值引用，也就是说 tuple 中的元素是可以修改的。
...
std::decay 也是个 Metafunction，类似我们前面讲的 remove_reference，它返回 T 的原始类型，这样无论 T 是引用类型，还是 const/volatile，都能保证用原始类型和后面的参数进行比较。通过 std::enable_if，当 dumps 参数的类型不是这些数值类型时，这个模板被从重载集中剔除。
...
我们定义了很多模板，但这时我们会遇到第一个问题就是名字查找的问题。如果我们直接把上面这些模板的定义写到头文件里，由于这些函数模板是相互调用的，例如 json::dumps<std::list<std::map<..., std::list<int>>>>(...) 会先调用 list 对应的 dumps，再调用 map 对应的 dumps，再调用 list 对应的 dumps，最后调用 int 对应的 dumps；所以这些模板之间的名字查找就存在了顺序依赖。比如 int 对应的 dumps 必须放在 list 对应的 dumps 之前，以保证 dumps<list> 能查找到 dumps<int>。但 list 和 map 在这个例子中是相互依赖的，把谁放前面都不行。所以，我们必须先前向声明（Forward Declare）所有的 dumps 模板。这和函数的前向声明是同一个道理，如果你熟悉名字查找问题的话应该不难理解。所以虽然我们在 Inclusion Model 中把模板的定义直接放在头文件里，但是模板的声明在某些情况下也是必不可少的。

我们已经处理了内置类型和 STL 中的类型，这时我们遇到的第二个问题就是，对于用户的自定义类型，我们应该怎么处理？比如一个 vector 里存了一个用户自定义的类型，json::dumps<vector<UserDefine>> 展开以后，怎么处理 UserDefine 的对象？这里就要用到依赖于实参的名字查找（Argument-dependent Name Lookup, ADL），ADL 是指，编译器会去函数实参类型所在的名字空间里去查找函数名字。所以，我们在定义 UserDefine 类型时，在同一个名字空间内提供一个针对 UserDefine 的 json::dumps 重载即可。这样，我们在 json.hpp 里定义的对应 vector 参数的函数模板，就也能查找到 UserDefine 的重载了。
